...
| 21 | **Marketing website** — *Phase A done, live* | **gidlist.com is live**, served by the `gidlist-site` Vercel project from `apps/site`; the GoDaddy parking page is gone. DNS: apex `A @ 216.198.79.1`, `CNAME www 4e153814e3021330.vercel-dns-017.com`, both TTL 600. The `app` CNAME and the five Resend email records were left untouched and verified still resolving. **Scope agreed:** a full product site — award-grade design, GSAP micro-animation, a WebGL moment where it earns its place, CMS-backed content, blog and knowledge base, all three languages, CTA straight to signup. **Phase A (done):** `apps/site`, tokens shared via `packages/design`, locale in the URL with `Accept-Language` negotiation and hreflang, all three locales prerendered static, security headers, `prefers-reduced-motion` honoured globally before any animation exists. **Phase B:** the page itself. **Phase C:** CMS on Supabase (`site_*` tables, RLS, the existing capability model — no new vendor). **Phase D:** blog, knowledge base, sitemap, structured data. **Open:** `www` serves the site instead of redirecting to the apex — harmless while canonicals point at the apex, but it should be a 308 redirect set in Vercel's Domains page. |
