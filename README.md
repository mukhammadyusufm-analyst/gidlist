...
| 28 | ~~**Item type: photo or file evidence**~~ | **Done.** `checklist_items.evidence` is `none`, `photo` or `file` — on the item, so it is versioned with the rest of the template and adding a photo requirement in March cannot change what a January submission was asked for. The answer side is `submission_items.evidence_path/_uploaded_at/_uploaded_by`. On a phone `capture="environment"` opens the rear camera directly, so there is no native app to write and item 29 does not wait on Expo either. **The bucket is private — the only one in this schema.** Logos and banners are public because they are decoration; a photograph of a shop floor is that customer’s operational data, and a public bucket makes it readable by anyone holding a guessable URL. Reading goes through signed URLs minted server-side in one batch call (an hour’s validity, one round trip rather than one per attachment). **The path is built server-side** as `<board_id>/<submission_id>/<answer_id>-<uuid>.<ext>` — the first segment is what the storage policy authorises on, so a client-chosen path would be a way into another company’s folder; the original filename is discarded apart from its extension. Replacing an attachment deletes the old object, and a failed row update deletes the object it just wrote, so nothing is orphaned in a bucket nobody lists. **The interface never says "verified":** it says "Photo attached", because a photograph proves something was photographed, not that it was photographed here or now. The evidence selector sits on the builder row because there is no item edit screen at all — items are added, reordered and deleted, nothing else; `updateItem` existed but nothing called it. **Not yet mandatory** — that is item 30, which carries the decisions this deliberately does not: what happens on a loading dock with no signal, whether a parent item can require it, and how a submission blocked for missing evidence differs from a missed one. |
...
| 30 | ~~**Require evidence before an item can be ticked**~~ | **Done.** `checklist_items.evidence_required` makes the attachment a condition rather than an invitation, alongside the location target from 29. **Enforced by a `before update` trigger on `submission_items`, not by the form** — ticking is a plain PostgREST update, so a check in TypeScript is a courtesy a crafted request walks straight past. **Parent items are exempt, per the decision:** a parent completes by rollup, so a requirement on it has no moment at which anybody could satisfy it and would deadlock the trigger doing the completing. The builder does not offer the choice on a parent either. The guard fires only on the transition to ticked, so commenting, attaching and un-ticking are untouched. `evidence_required` cannot be set while `evidence` is `none` — refused by the schema and disabled in the form. **Open, and now queued separately as item 36:** an optional submit-by deadline judged on when items were *ticked* rather than when the submission was sent, which is what makes filling in offline and syncing later workable. |
