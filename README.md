...
| 15a | **Alerting on the nightly job** | Still open. If `cron.schedule` stops, obligations silently stop appearing and a customer finds out first. `onRequestError` cannot see this — the job fails inside Postgres, not inside a request. Needs a check that the last materialisation ran, and somewhere to say so. |
