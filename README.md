...
| 21 | **Marketing website** — *Phases A and B done, live* | **gidlist.com is live** in all three languages, served by the `gidlist-site` Vercel project from `apps/site`. DNS: apex `A @ 216.198.79.1`, `CNAME www 4e153814e3021330.vercel-dns-017.com`, both TTL 600; the `app` CNAME and the five Resend email records were left untouched and verified still resolving. **Phase A:** the app, tokens shared via `packages/design`, locale in the URL with `Accept-Language` negotiation and hreflang, security headers. **Phase B (done):** hero, problem, what-it-does, how-it-works, pricing and closing CTA, all three locales prerendered static. Scroll reveals are pure CSS `animation-timeline: view()` — no JavaScript, so a failed bundle cannot leave the page blank, and browsers without support simply show the content. Pricing is hand-mirrored from the `plans` table in `apps/site/src/lib/pricing.ts`; **that file drifts silently if a price changes in SQL** until Phase C. Plan names are deliberately untranslated because `plans.name` is a single column the product shows in English to everyone. **Phase C:** CMS on Supabase (`site_*` tables, RLS, the existing capability model — no new vendor), which also retires the hand-mirrored pricing. **Phase D:** blog, knowledge base, sitemap, structured data. **Deferred from B:** GSAP micro-animation and the WebGL moment, and a CSP (the site has security headers but no CSP; it needs a per-request nonce in `proxy.ts`, as the product does, and should land with the animation work). **Open:** `www` serves the site instead of 308-redirecting to the apex — harmless while canonicals point at the apex, but it should be set in Vercel’s Domains page. |
...
| 25 | **Per-currency pricing (`plan_prices`)** | `plans` carries a single `price_minor` and `currency`, so the product can only be priced in one currency. Selling in UZS locally and USD internationally needs `plan_prices (plan_code, currency, price_minor)` — the pattern `addon_prices` already uses for per-plan add-on pricing. **Set the UZS figures deliberately, do not convert:** $5 converted lands on something like 61,437 UZS, which reads as a foreign price with the exchange rate showing through. Round local numbers also let the product be priced for the local market rather than exporting US pricing into it. Blocks the marketing site’s pricing section (21, Phase B) showing local currency. Current USD tiers: Free $0 / Starter $5 / Team $15 / Business $40, with 5/10/40/150 members and 1/2/5/15 spaces. |
| 26 | **Gateway selection by currency** | `activeGateway()` reads one `PAYMENT_PROVIDER` environment variable and returns a single gateway, so one deployment can serve only one provider. It needs to resolve per customer — `gatewayFor(currency)` — so an Uzbek customer reaches Payme or Click and a German one reaches the international provider, from the same deployment. The `PaymentGateway` interface already carries a `currencies` array for exactly this. Prerequisite for 22. |
| 27 | **No way to edit prices without SQL** | `plans`, `plan_features`, `addons` and `addon_prices` have RLS enabled with **read-only** policies — `for select to public using (true)` and no write policy — so no signed-in user, the owner included, can change a price through the app. There is also no admin screen for it. Read-only is the right default for prices, but the combination is the same shape as the archiving and audit-log gaps: a mechanism with no way to reach it. Wants a plans editor behind a `billing` platform capability. |
