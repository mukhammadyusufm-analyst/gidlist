...
| 5 | ~~**Checklist preview**~~ | **Done.** On the Details tab, showing the checklist as its filler will read it. Previews the draft when there is one — that is the version about to be committed to — and the published one otherwise, so it is never empty on a checklist in use. A separate presentational component rather than the real fill sheet in read-only mode: that one is bound to a submission id, and reusing it would mean inventing a submission and putting a row in the compliance record for a checklist nobody filled in. |
...
| 18 | **Full visual checklist preview** | Item 5 shows structure only. The need is to see it as the filler actually sees it — banner, avatar, section styling, the real fill-in layout. That means rendering the genuine fill sheet against a throwaway in-memory submission rather than a second presentational component, which is the trade item 5 deliberately took and this one has to undo. Needs a way to construct a submission-shaped object that is never persisted. |
| 19 | **Brandbook** | The palette already carries an implicit identity — blue-violet primary at oklch hue 262, the CircleCheckBig mark, Inter. A brandbook makes it deliberate and writes down logo use, colour roles, type scale, tone of voice in three languages, and what the product is called in each. Everything brand-related should be built from it rather than beside it. Worth doing before the marketing site (21), which would otherwise invent a second identity. |
...
| 21 | **Marketing website** | On gidlist.com, which today serves the registrar's parking page and fails on HTTPS. Separate from the app at pp.gidlist.com — see the decision block on subfolders. Should follow the brandbook (19), and is where locale-in-URL (2d) pays off for SEO. |
