...
| 17 | **`submissions` retention** — *attachments done, rows still open* | **The expensive half is done.** Rows are cheap and files are not: a submission row is a couple of hundred bytes, a phone photograph is three megabytes, and a daily checklist with four photo items is roughly fifteen hundred files and five gigabytes a year per customer. **The record is never deleted** — that is the product, and "a record nobody can quietly edit" cannot carry a footnote about disappearing after a year. What expires is the file: `photo_expired_at` / `file_expired_at` keep the fact that evidence existed, and the fill sheet says "an attachment was provided, and the file was removed on ‹date›" rather than showing nothing. Retention is `plans.evidence_retention_days` — 90 / 365 / 730 / indefinite — on the plan because it is exactly what a bigger tier should buy, and a space with no subscription falls back to the free window, the safe direction to be wrong in. A nightly `expire_evidence()` job runs at 02:30 UTC. **The remaining piece, and it is not cosmetic:** removing a row from `storage.objects` does not remove the file from object storage, which needs the Storage API that SQL cannot call. So expiry writes every path to `storage_cleanup_queue` and nulls the reference — nothing is ever forgotten — but **until a privileged drain runs, the files still exist and are still billed**. The drain needs the service-role key, which the app does not yet hold; the same capability the billing webhooks will need. **Also still open:** retention for the submission rows themselves, which grow forever and which the compliance dashboard reads. That is a partitioning question rather than a deletion one, since the rows are the record. |
