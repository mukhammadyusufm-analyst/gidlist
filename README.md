...
| 35 | **A submission does not record who filled it in** | **The most serious item in this group, because it undercuts the product’s central claim.** `submissions` has `assignee_id` and `assignee_email` — who was *asked* — and no column for who *did it*. The compliance table therefore shows the assignee, and where a schedule named nobody it shows **“Anyone”**, which is what the schema intends: the column comment says null means "anyone on the board". So a checklist filled in by a named person is reported as filled by nobody in particular. A record that cannot say who did the work is not the record an auditor is being promised. **Three separate changes.** (a) Add `submitted_by` (and `first_filled_by`, since ticking starts before submitting) written by the database, never by the client. (b) Show *that* in compliance instead of the assignee, keeping the assignee as its own column — they are different questions and conflating them is what produced this. (c) Decide what an unassigned schedule means: refuse to publish without an assignee, default to the creator, or allow "any member of this space" as an explicit, named choice rather than as the absence of one. **Note (c) alone does not fix it** — assigning to a whole space still leaves the record naming a group, so (a) is required regardless. Existing rows cannot be back-filled: nobody recorded it, and inventing a name would be worse than the gap. |
