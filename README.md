...
| 23 | **The app's root page is still the Phase 0 placeholder** | `app.gidlist.com/` serves `apps/web/src/app/page.tsx`, written in Phase 0 and never revisited — a centred heading, two buttons, none of the current design. Not a regression and not a build problem: `/login` and `/signup` both carry the redesigned two-column shell, the CSP header is live with a per-request nonce, and every stylesheet and font returns 200. The root simply predates all of it. The file's own comment already says the real marketing page belongs on the main site rather than inside the app, which points at the fix: the app subdomain should not host a landing page at all — send signed-out visitors straight to `/login` and signed-in ones to `/dashboard`, and let gidlist.com (21) do the selling. Do it with 21 so the two do not contradict each other. |
