...
| 33 | **Group the marketing-copy editor by where the text appears** | Admin → Marketing site is a flat alphabetical list of dotted keys — `features.0.title`, `problemCards.1.body`, `pricingLead`. It is complete and it is hostile: to change the hero you have to know that the hero is called `headline` and `subhead`. Group the rows by section in the order they appear on the page — Hero, Problem, What it does, How it works, Pricing, Footer — with the section name as a heading. The key stays visible underneath, because it is what the database stores and what an error message would name. `siteContentKeys()` derives the list from the English catalogue, so grouping means giving the catalogue a declared section order rather than hand-listing keys in a second place, or the two drift the first time a string is added. |
| 34 | **The installed app’s launch screen is ugly** | Android builds the splash from the manifest: `background_color` behind the icon it picks. It chose the maskable icon, which is a hard-edged full-bleed square by design, and set it against `#fbfbfc` — so a sharp blue square sits on near-white with no relationship to either. Options, roughly in order of effort: give `background_color` the brand blue so the square dissolves into the field; or supply a dedicated non-maskable icon with generous padding so it reads as a mark rather than a tile. iOS ignores the manifest for this and uses `apple-touch-startup-image`, so it needs its own asset or it shows a white screen. Worth doing before the award submission, since the launch screen is the first frame anybody sees of the installed app. |
| 35 | **A submission does not record who filled it in** — *(a) and (b) done* | **Done:** `submissions.submitted_by` and `submitted_by_email`, written only by `submit_submission()` from `auth.uid()` inside a SECURITY DEFINER function — never from an argument, because a client-supplied name on the row that proves who did the work would be a signature anybody could forge. The email is denormalised at submission time, matching the `assignee_email` pattern: a record of who signed off should not change because somebody later changed their address. Compliance now has a **Filled by** column *beside* the assignee, not instead of it — "who was asked" and "who did it" are different questions, and answering the second with the first is what made a checklist filled in by a named person read as "Anyone". **Not back-filled, deliberately:** nobody recorded who completed the existing rows, and a plausible guess would be invented evidence in a compliance history; those rows show an em dash, as does anything not yet submitted. Note the detail was never entirely absent — `submission_items.checked_by` has recorded who ticked each item since Phase 4; what was missing was the roll-up and somewhere to show it, which also leaves a per-item drill-down available later. **Still open (c):** what an unassigned schedule should mean. Refuse to publish without an assignee, default to the creator, or make "any member of this space" an explicit named choice rather than the absence of one. Worth noting (c) was never sufficient on its own — assigning to a whole space still leaves the record naming a group, which is why (a) came first. |
