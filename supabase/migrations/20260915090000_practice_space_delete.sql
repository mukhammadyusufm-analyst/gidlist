...
-- In production, run in order: getting-started, getting-started-language,
-- getting-started-settings, then this.
-- =============================================================================
-- The practice space can be deleted — once its lesson has been learned.
--
-- The rule (15 Sep 2026): the practice space may be deleted once its owner has
-- their own space with a checklist on a schedule. Until then it is the only
-- working example they have, so it stays; after that it is clutter, and
-- deleting it leaves them with the one space they actually use.
--
--   * practice_space_can_be_deleted(board) answers the question, for the button.
--   * delete_practice_space(board) enforces the same rule and deletes.
--   * The practice checklist gets a last item saying when and where to do it.
--
-- DELETING A SPACE THAT HAS SUBMISSIONS, SAFELY. submissions.checklist_version_id
-- is ON DELETE RESTRICT, and a cascade from the space reaches versions and
-- submissions in an order Postgres does not promise, so it can refuse halfway.
-- The submissions therefore go first, explicitly, then the space. Photos and
-- files attached in the practice checklist are queued in storage_cleanup_queue
-- first — the queue retention already drains — so nothing is left in storage.
-- delete_account() now uses the same routine for an account's practice space.
-- =============================================================================

begin;

-- Internal: remove one practice space and everything in it. Called only by the
-- functions below, which make the permission decisions.
create or replace function public.purge_practice_board(p_board_id uuid)
returns void
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  delete from public.boards where id = p_board_id and is_tutorial;
end;
$$;

revoke execute on function public.purge_practice_board(uuid) from public, anon, authenticated;

create or replace function public.practice_space_can_be_deleted(p_board_id uuid)
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
     ...
     and exists (
           select 1
             from public.boards own
             join public.checklists c on c.board_id = own.id and c.archived_at is null
             join public.schedules sc on sc.checklist_id = c.id and sc.active
            where own.owner_id = (select auth.uid())
              and not own.is_tutorial
              and own.archived_at is null
         );
$$;

revoke execute on function public.practice_space_can_be_deleted(uuid) from public, anon;
grant execute on function public.practice_space_can_be_deleted(uuid) to authenticated;

create or replace function public.delete_practice_space(p_board_id uuid)
returns void
language plpgsql
security definer
set search_path = ''
...
begin
  if not exists (
    select 1 from public.boards b
     where b.id = p_board_id and b.is_tutorial and b.owner_id = (select auth.uid())
  ) then
    raise exception 'Only the owner can remove a practice space.'
      using errcode = 'insufficient_privilege';
  end if;

  if not public.practice_space_can_be_deleted(p_board_id) then
    raise exception 'The practice space can be removed once you have your own space with a checklist on a schedule.'
      using errcode = 'check_violation';
  end if;

  perform public.purge_practice_board(p_board_id);
end;
$$;

revoke execute on function public.delete_practice_space(uuid) from public, anon;
grant execute on function public.delete_practice_space(uuid) to authenticated;

-- delete_account(): the practice space goes through the same safe routine.
...
begin
  ...
  for v in select b.id from public.boards b where b.owner_id = p_user_id and b.is_tutorial loop
...
-- The practice checklist, with its last item: when and where to delete it.
...
begin
  ...
  s := case v_locale
    when 'uz' then jsonb_build_object(
      ...
      'i10', 'Tayyor boʻlgach, bu mashq maydonini oʻchiring',
      'i10d', 'Oʻz maydoningizda jadvalga qoʻyilgan roʻyxat paydo boʻlgach, shu maydonning sozlamalarini ochib, «Mashq maydonini oʻchirish» tugmasini bosing. Sizda faqat oʻz maydoningiz qoladi.'
    )
    when 'ru' then jsonb_build_object(
      ...
      'i10', 'Когда будете готовы, удалите это учебное пространство',
      'i10d', 'Когда в вашем собственном пространстве появится чек-лист с расписанием, откройте настройки этого пространства и нажмите «Удалить учебное пространство». У вас останется только ваше пространство.'
    )
      ...
      'i10', 'Delete this practice space when you are ready',
      'i10d', 'Once your own space has a checklist on a schedule, open Settings in this space and choose Delete practice space. You are left with your own space.'
    )
  end;
...
-- STATE CHECK — expect one row: t | t | t
--   can_check, can_delete, tutorial_mentions_deleting
select
  to_regprocedure('public.practice_space_can_be_deleted(uuid)') is not null as can_check,
  to_regprocedure('public.delete_practice_space(uuid)') is not null        as can_delete,
  pg_get_functiondef('public.ensure_getting_started(text)'::regprocedure) like '%i10d%' as tutorial_mentions_deleting;
