...
export function applySiteOverrides(
  ...
  const next: SiteMessages = {
    ...base,
    problemCards: base.problemCards.map((c) => ({ ...c })) as SiteMessages['problemCards'],
    features: base.features.map((c) => ({ ...c })) as SiteMessages['features'],
    steps: base.steps.map((c) => ({ ...c })) as SiteMessages['steps'],
  };
  ...
  for (const [key, value] of Object.entries(overrides)) {
    ...
    // matches the catalogue, and the bundled copy is the safer answer.
