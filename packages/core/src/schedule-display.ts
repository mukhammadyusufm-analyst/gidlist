/**
 * The parts of a schedule that a browser needs, with no validation attached.
 *
 * Split out of `schedules.ts` for one reason: that file builds Zod schemas at
 * module scope, so a component importing `describeSchedule` to render a line of
 * text pulled Zod and every schema in this package into the page. Nothing here
 * imports Zod, so a `'use client'` component can import
 * `@app/core/schedule-display` and pay for what it actually uses. Server code
 * carries on importing from the barrel, where nothing is saved by being fussy.
 *
 * The rule for adding to this file: constants and pure formatting only. A
